package wdwd.com.booklib.view.weight;
...
import wdwd.com.booklib.util.ViewDragHelper;
...
//        Log.e("cyy","onTouchEvent"+ev.getAction());
        if (!dispatchingChildrenContentView) {
            processEvent(ev);
//            Log.e("cyy","processEvent");
        }
        ...
        if (ev.getAction() == MotionEvent.ACTION_MOVE && ratio == 0) {
            ...
            if (!dispatchingChildrenDownFaked) {//如果是首次传递事件给RecyclerView
                ...
                Log.e("cyy", "记录++" + dispatchingChildrenStartedAtY);
        ...
        if (dispatchingChildrenContentView && dispatchingChildrenStartedAtY < ev.getY()) {
            Log.e("cyy", "dispatchingChildrenContentView " + dispatchingChildrenStartedAtY + "+++++" + ev.getY() + "+++" + dispatchingChildrenContentView);
